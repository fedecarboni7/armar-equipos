...
def validate_ai_response(result: dict, available_players: list[dict]) -> dict:
        ...
        validated_matches.append(
            {
                ...
                "matched_player_name": str(matched_player_name)
    ...
    return {"matches": validated_matches, "not_found": [str(n) for n in not_found if n]}
...
async def match_players(input_names: list[str], available_players: list[dict]) -> dict:
    ...
    players_formatted = "\n".join(
        [f"- ID: {p['id']}, Nombre: {p['name']}" for p in available_players]
    )
    ...
    result = await chain.ainvoke(
        {"input_names": names_formatted, "available_players": players_formatted}
    )
