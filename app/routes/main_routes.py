...
async def build_teams_api(
    ...
    try:
        ...
        if club_id:
            ...
            all_players = execute_with_retries(
                query_players, db, current_user_id, scale=scale
            )
        ...
        if len(selected_players) != len(selected_player_ids):
            return JSONResponse(
                content={"error": "Algunos jugadores no fueron encontrados"},
                status_code=400,
            )
...
async def match_players_api(
    ...
    try:
        ...
        if not isinstance(input_names, list):
            return JSONResponse(
                content={"error": "input_names debe ser una lista"}, status_code=400
            )

        if not input_names:
            return JSONResponse(
                content={"error": "No se proporcionaron nombres"}, status_code=400
            )
        ...
        if len(input_names) > MAX_LINES:
            return JSONResponse(
                content={
                    "error": f"Se permiten máximo {MAX_LINES} líneas. Por favor, reduce la lista."
                },
                status_code=400,
            )
        ...
        if not all_players:
            return JSONResponse(
                content={"error": "No hay jugadores disponibles"}, status_code=400
            )
