...
def test_post_signup(client, db):
    ...
    response = client.post(
        "/signup",
        data={"username": "newuser2", "password": password, "email": email},
        follow_redirects=False,
    )
    assert response.status_code == 409
...
def test_create_token(client, token_user):
    response = client.post(
        "/token",
        data={"username": token_user["username"], "password": token_user["password"]},
        follow_redirects=False,
    )
    assert response.status_code == 200
...
def test_token_with_wrong_password(client, token_user):
    response = client.post(
        "/token",
        data={"username": token_user["username"], "password": "wrongpassword"},
        follow_redirects=False,
    )
    assert response.status_code == 401
...
def test_token_with_wrong_username(client, token_user):
    response = client.post(
        "/token",
        data={"username": "wronguser", "password": token_user["password"]},
        follow_redirects=False,
    )
    assert response.status_code == 401
    assert response.json()["detail"] == "Incorrect username or password"
...
def test_token_with_empty_username(client):
    response = client.post(
        "/token", data={"password": "tokenpassword"}, follow_redirects=False
    )
    assert response.status_code == 422
    assert response.json()["detail"][0]["msg"] == "Field required"
...
def test_token_with_empty_password(client):
    response = client.post(
        "/token", data={"username": "tokenuser"}, follow_redirects=False
    )
    assert response.status_code == 422
    assert response.json()["detail"][0]["msg"] == "Field required"
...
def test_forgot_password_invalid_email(client):
    ...
    response = client.post(
        "/forgot-password", data={"email": "invalid-email"}, follow_redirects=False
    )
    assert response.status_code == 200
    assert "error" in response.context
...
def test_profile_update_email(client, db):
    ...
    response = client.post(
        "/login",
        data={"username": username, "password": "Testpassword1*"},
        follow_redirects=False,
    )
    assert response.status_code == 302

    response = client.post(
        "/perfil/update-email", data={"email": new_email}, follow_redirects=False
    )
    assert response.status_code == 200
    assert "success" in response.context
...
def test_profile_update_email_duplicate(client, db):
    ...
    response = client.post(
        "/login",
        data={"username": "user2", "password": "Testpassword1*"},
        follow_redirects=False,
    )
    assert response.status_code == 302
...
def test_login_with_unconfirmed_email(client, db):
    ...
    assert response.context["email_not_confirmed"]
...
def test_resend_confirmation_email(client, db):
    ...
    response = client.post(
        "/resend-confirmation", data={"email": email}, follow_redirects=False
    )
    assert response.status_code == 200
    assert response.template.name == "email_confirmation_pending.html"
...
def test_resend_confirmation_email_nonexistent(client):
    ...
    assert response.template.name == "email_confirmation_pending.html"
    assert (
        "recibirás un nuevo enlace" in response.context["success"]
...
def test_resend_confirmation_already_confirmed(client, db):
    ...
    assert (
        "recibirás un nuevo enlace" in response.context["success"]
    )  # Security: same message for all cases
...
def test_profile_resend_email_confirmation(client, db):
    ...
    assert response.context["email_pending_confirmation"]
